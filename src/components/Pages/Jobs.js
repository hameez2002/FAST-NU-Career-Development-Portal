...
export const Jobs = () => {
  ...
  const handleJobSubmit = async (formData) => {
    try {
      ...
      // await axios.post("http://localhost:7000/jobs", formData);
      await axios.post("https://backend-fast-nu-career-development-portal-yw26-6ab691fsb.vercel.app/jobs", formData);

      // Fetch the newly added job from the server
      // const response = await axios.get("http://localhost:7000/jobs");
      const response = await axios.get("https://backend-fast-nu-career-development-portal-yw26-6ab691fsb.vercel.app/jobs");
    ...
    if (selectedJob && selectedJob.job_id) {
      try {
        const response = await axios.delete(
          ...
          `https://backend-fast-nu-career-development-portal-yw26-6ab691fsb.vercel.app/jobs/${selectedJob.job_id}`
        );
